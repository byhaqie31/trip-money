...
// The signature share card. Ivory pass on the dark forest panel, all data in
// Geist Mono with tiny uppercase labels, per the handoff. The timing signal
// line rides on the pass (v2 brief requirement, handoff language).
const { destination, budgetRm, budgetSen } = useTrip()
...
async function save(): Promise<void> {
  ...
  try {
    ...
    a.href = dataUrl
    a.download = `trip-money-KUL-${destination.value.airport}.png`
    a.click()
    saved.value = true
...
<template>
  <section class="rounded-panel bg-pass-panel px-6 pb-[26px] pt-7">
    ...
    <div ref="passEl" class="rounded-pass bg-pass-paper text-pass-ink shadow-pass [animation:var(--animate-pass-in)]">
      <div class="flex items-baseline justify-between px-[22px] pt-5">
        ...
        <div class="font-mono text-[9px] tracking-[0.14em] text-pass-ink/50">BOARDING PASS</div>
      </div>
      ...
      <div class="flex items-center gap-4 px-[22px] pt-[18px]">
        ...
        <div class="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div class="text-[13px] text-pass-ink/60">→</div>
          <div class="h-px flex-1 bg-pass-ink/25" />
        </div>
      ...
      <div class="grid grid-cols-2 gap-x-3 gap-y-4 px-[22px] pb-5 pt-[22px]">
        <div>
          <div class="mb-1 font-mono text-[8.5px] uppercase tracking-[0.14em] text-pass-ink/50">Traveller budget</div>
          <div class="font-mono text-[16px] font-medium">RM {{ budgetRm.toLocaleString('en-MY') }}.00</div>
        </div>
        <div class="text-right">
          <div class="mb-1 font-mono text-[8.5px] uppercase tracking-[0.14em] text-pass-ink/50">Converted</div>
        ...
        <div>
          <div class="mb-1 font-mono text-[8.5px] uppercase tracking-[0.14em] text-pass-ink/50">Middle rate</div>
          <div class="font-mono text-[13px]">
            <template v-if="latest">{{ fmtRate(latest.rate) }}</template>
          </div>
        </div>
        <div class="text-right">
          <div class="mb-1 font-mono text-[8.5px] uppercase tracking-[0.14em] text-pass-ink/50">Date</div>
          <div class="font-mono text-[13px]">
            <template v-if="asOfDate">{{ fmtPassDate(asOfDate) }}</template>
          </div>
        </div>
        <div v-if="percentile != null" class="col-span-2">
          <div class="mb-1 font-mono text-[8.5px] uppercase tracking-[0.14em] text-pass-ink/50">Timing</div>
          <div class="font-mono text-[11px]">Better than {{ percentile }}% of the last 90 days</div>
        </div>
      </div>

      <div class="relative h-0 border-t-2 border-dashed border-pass-ink/25">
      ...
      <div class="flex items-center gap-4 px-[22px] pb-[18px] pt-4">
        <div class="h-9 flex-1 rounded-[2px] opacity-85 [background:repeating-linear-gradient(90deg,var(--color-pass-ink)_0_2px,transparent_2px_4px,var(--color-pass-ink)_4px_5px,transparent_5px_9px,var(--color-pass-ink)_9px_12px,transparent_12px_14px)]" />
        <div class="max-w-[60%] flex-none text-right">
          ...
          <div class="mt-[3px] font-mono text-[8.5px] text-pass-ink/50">Trip Money · rates by BNM via data.gov.my</div>
        </div>
      </div>
    </div>
